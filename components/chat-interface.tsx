...
export default function ChatInterface({ selectedPlaylists, onGeneratePlaylist }: ChatInterfaceProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const wantsGenerate =
            ...
            lower.includes("create playlist") ||
            lower.includes("build it") ||
            lower.includes("add it") ||
            lower.includes("save to spotify");
        ...
        <section className="w-full pt-12">
            <div className="flex items-center justify-between mb-4">
                ...
                <Button
                    ...
                    size="sm"
                    onClick={handleReset}
                    disabled={resetPending || chatMutation.isPending}
                    className="gap-2"
            ...
            <Card className="border border-border overflow-hidden  " >
                    ...
                    {messages.map((message: ChatMessage, _index: number) => (
                        <div key={message.id} className={`chat-bubble flex gap-3 ${message.role === 'user' ? 'justify-end' : ''}`}>
                            ...
                            <div className={`flex-1 ${message.role === 'user' ? 'flex flex-col items-end' : ''}`}>
                                <div className={`rounded-2xl p-4 max-w-lg ${message.role === 'user'
                ...
                <div className="border-t border-border p-4">
                    ...
                    <p className="text-xs text-muted-foreground mt-2 ml-4">
                        AI-powered by OpenAI • Your data stays private
                    </p>
                </div>
            </Card>
        </section>
