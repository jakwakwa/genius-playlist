...
export default function GeneratedPlaylist({ playlist }: GeneratedPlaylistProps) {
                ...
                description: (
                    <div className="flex items-center gap-2">
                        ...
                        <Button
                            variant="outline"
                            size="sm"
                            onClick={() => window.open(data.spotifyUrl, '_blank')}
                        >
    ...
    return (
        <section>
            <div className="flex w-screen items-center justify-between gap-4 mt-28 mb-12
            ">
                <h3 className="text-xl font-bold">Generated Playlist Preview</h3>
                <Button
                    onClick={() => saveToSpotifyMutation.mutate()}
                    disabled={saveToSpotifyMutation.isPending}
                    className="flex items-center gap-2"
                    data-testid="button-save-spotify"
                >
            ...
            <Card className="border border-border overflow-hidden">
                ...
                <div className="p-6 bg-primary">
                    ...
                    {displayedTracks.map((track, index) => (
                        <div
                            key={track.id || index}
                            className="track-row flex items-center gap-6 py-3 px-2 rounded-md cursor-pointer group hover:bg-card/50 transition-colors"
                    ...
                    {playlist.tracks.length > 8 && (
                        <div className="mt-6 pt-6 border-t border-border text-center">
                            <Button
                                ...
                                data-testid="button-toggle-tracks"
                            >
                                {showAllTracks ? (
                                    <>
