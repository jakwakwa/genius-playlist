...
function Sidebar({
	...
	if (collapsible === "none") {
		return (
			<div data-slot="sidebar" className={cn("bg-[#000] text-sidebar-foreground  border-none flex h-full w-(--sidebar-width) flex-col", className)} {...props}>
				{children}
			</div>
		);
	}
		...
		return (
			<Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
				<SheetContent
					...
					<div className="flex h-fit w-screen flex-col">{children}</div>
				</SheetContent>
			</Sheet>
		);
	}
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
	...
	return (
		<button
			...
			className={cn(
				"hover:after:bg-none absolute inset-y-0 z-90 hidden w-1 p-0 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 sm:flex",
				"in-data-[side=left]:cursor-w-resize in-data-[side=right]:cursor-e-resize",
				"[[data-side=left][data-state=collapsed]_&]:cursor-e-resize [[data-side=right][data-state=collapsed]_&]:cursor-w-resize",
				"hover:group-data-[collapsible=offcanvas]:bg-none group-data-[collapsible=offcanvas]:translate-x-0 group-data-[collapsible=offcanvas]:after:left-full",
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
	return (
		<main
			...
			className={cn(
				"relative overflow-x-hidden layout-inset-background flex w-full flex-1 flex-col",
...
function SidebarSeparator({ className, ...props }: React.ComponentProps<typeof Separator>) {
	return <Separator data-slot="sidebar-separator" data-sidebar="separator" className={cn("bg-[#ffffff0c]  w-auto", className)} {...props} />;
}
...
function SidebarMenuButton({
	...
	const button = (
		<Comp data-slot="sidebar-menu-button" data-sidebar="menu-button" data-size={size} data-active={isActive} className={cn(sidebarMenuButtonVariants({ variant, size }), className)} {...props}></Comp>
	);
